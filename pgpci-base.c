...
#else
static void pgpci_tty_at_set_termios(struct tty_struct *tty, struct termios *old_termios);
#endif
...
#else
            if (!(board->tty_at_channels[i] = polygator_tty_device_register(&pdev->dev, mod, &pgpci_tty_at_ops))) {
#endif
